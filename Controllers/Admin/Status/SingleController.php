...
use Benefits\Models\Update\SinglePatchesObserver;
use Benefits\Models\Update\SingleUpdatesObserver;
...
final class SingleController
{
    ...
    private function processUpdate()
    {
        ...
        if($this->conf->isNetworkEnabled() === FALSE)
        {
            ...
            if(version_compare($currentPluginSemverInDatabase, $latestSemver, '=='))
            {
                // It's a last version
                $semverUpdated = TRUE;
            }
            ...
            if(version_compare($updatedPluginSemverInDatabase, '6.1.0', '>=') && version_compare($updatedPluginSemverInDatabase, '6.2.0', '<'))
            {
                // Run 6.1.Z patches
