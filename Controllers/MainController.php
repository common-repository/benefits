...
use Benefits\Models\Load\AutoLoad;
...
final class MainController
{
    ...
    public function modifyNetworkActionLinks(array $paramExistingNetworkActionLinks)
    {
        ...
        if($this->canProcess)
        {
            try
            {
                ...
                $objStatus = new \Benefits\Models\Status\NetworkStatus($conf, $lang);
    ...
    public function modifyActionLinks(array $paramExistingActionLinks)
    {
        ...
        if($this->canProcess)
        {
            try
            {
                ...
                $objStatus = new \Benefits\Models\Status\SingleStatus($conf, $lang, $conf->getBlogId());
    ...
    public function multisiteActivate()
    {
        try
        {
            ...
            $objInstaller = new \Benefits\Controllers\Admin\InstallController($conf, $lang, $conf->getBlogId());
        ...
        if($this->canProcess)
        {
            try
            {
                ...
                if($conf->isNetworkEnabled())
                {
                    ...
                    $lang = new \Benefits\Models\Language\Language(
                    ...
                    $objUninstaller = new \Benefits\Controllers\Admin\InstallController($conf, $lang, $paramBlogIdToDelete);
    ...
    public function uninstall()
    {
        if($this->canProcess)
        {
            try
            {
                ...
                if($conf->isNetworkEnabled())
                {
                    $objNetworkUninstaller = new \Benefits\Controllers\Admin\InstallController($conf, $lang, $conf->getBlogId());
                    $sites = get_sites();
                    foreach ($sites AS $site)
                    {
                        $blogId = $site->blog_id;
                        ...
                        $objUninstaller = new \Benefits\Controllers\Admin\InstallController($conf, $lang, $blogId);
                        $objUninstaller->deleteContent();
                        // INFO: This plugin does not have it's own plugin-specific roles
                        $objUninstaller->removeCustomCapabilities();
                    }
                ...
                } else
                {
                    ...
                    $objUninstaller = new \Benefits\Controllers\Admin\InstallController($conf, $lang, $conf->getBlogId());
                    $objUninstaller->deleteContent();
                    // INFO: This plugin does not have it's own plugin-specific roles
                    $objUninstaller->removeCustomCapabilities();
    ...
    public function loadNetworkAdmin()
    {
        if($this->canProcess)
        {
            try
            {
                ...
                // Create mandatory instance
                $objAssetController = new \Benefits\Controllers\Admin\AssetController($conf, $lang);
                $objMenuController = new \Benefits\Controllers\Admin\NetworkMenuController($conf, $lang);
    ...
    public function loadAdmin()
    {
        if($this->canProcess)
        {
            try
            {
                ...
                $objAssetController = new \Benefits\Controllers\Admin\AssetController($conf, $lang);
                $objStatus = new \Benefits\Models\Status\SingleStatus($conf, $lang, $conf->getBlogId());
                $objMenuController = new \Benefits\Controllers\Admin\SingleMenuController($conf, $lang);
    ...
    public function runOnInit()
    {
        if($this->canProcess)
        {
            try
            {
                ...
                $objStatus = new \Benefits\Models\Status\SingleStatus($conf, $lang, $conf->getBlogId());
                $objAssetController = new \Benefits\Controllers\Front\AssetController($conf, $lang);
    ...
    public function parseShortcode($attributes = array())
    {
        ...
        if($this->canProcess)
        {
            try
            {
                ...
                // Create mandatory instances
                $objStatus = new \Benefits\Models\Status\SingleStatus($conf, $lang, $conf->getBlogId());
                $objShortcodeController = new \Benefits\Controllers\Front\ShortcodeController($conf, $lang);
     ...
     *
     * @access private
     * @return null|\Benefits\Models\Configuration\ConfigurationInterface
     * @throws \Exception
     */
    private function conf()
    {
        ...
        if(is_null($this->conf) && !is_null($this->confWithoutRouting) && static::$dependenciesLoaded === TRUE)
        {
            ...
            $routing = new \Benefits\Models\Routing\UI_Routing($pluginPath, $pluginURL, $themeUI_FolderName);
     ...
     *
     * @access private
     * @return null|\Benefits\Models\Language\Language
     * @throws \Exception
     */
